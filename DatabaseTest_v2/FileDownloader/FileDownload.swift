...
public class FileDownload
{
   ...
   public var cachedFilePath: String? {
            get{
            if var cacheEntry = cacheEntry(),
                let fileName = cacheEntry["fileName"]{
     ...
     */
    public func download(completion:FileDownloadManager.DownLoadCompletionHandler)->Void {
    ...
    public func cacheEntry() -> [String:String]?{
        
    
        if let cachedEntry:NSDictionary = FileDownloadManager.sharedInstance.cache.objectForKey(self.url){
            
            
             print(FileDownloadManager.sharedInstance.cache.objectForKey(self.url))
           
            
            if let fileName = cachedEntry["fileName"] as? String{
                self.thisFileName = fileName
                
                print("filnavnet er: "+thisFileName)
                var filePath = FileDownloadManager.cacheDirectory;
                filePath = (filePath as NSString).stringByAppendingPathComponent(fileName)
                return ["filePath" : filePath, "lastModified": cachedEntry["lastModified"] as! String, "ETag" : cachedEntry["ETag"] as! String,"newFileName":fileName ]
